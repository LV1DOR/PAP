...
export default function TownCard({ town, onClick }) {
  if (!town) return null;
  const { name, slug } = town;
  const stats = town.stats || { count: 0, active: 0, last_report_at: null };
  return (
    <Card
    ...
    >
      ...
      <div className="flex gap-4 text-sm">
        <div><span className="font-medium">Total:</span> {stats.count ?? 0}</div>
        <div><span className="font-medium">Active:</span> {stats.active ?? 0}</div>
      </div>
