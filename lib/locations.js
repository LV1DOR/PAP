...
export async function fetchLocationsWithStats() {
  ...
  // For each location fetch aggregated stats (N+1 for now; optimize later with RPC or view)
  const result = [];
  for (const l of locs) {
    const { data: reportsData, error: repErr } = await supabase
      ...
      .limit(100); // cap for initial stats

    if (repErr) {
      result.push({ ...l, stats: { count: 0, active: 0, last_report_at: null } });
      continue;
    }
    const count = reportsData.length;
    const active = reportsData.filter(r => r.status !== 'resolved' && r.status !== 'rejected').length;
    const last_report_at = reportsData[0]?.created_at || null;
    result.push({ ...l, stats: { count, active, last_report_at } });
  }
...
export async function fetchReportsByLocation(slug, { limit = 200, withImages = true } = {}) {
  // Resolve slug -> location
  const { data: locData, error: locErr } = await supabase
    .from('locations')
    .select('*')
    .eq('slug', slug)
    .single();
  if (locErr || !locData) return { error: locErr?.message || 'Location not found', reports: [], location: null };

  let query = supabase
    .from('reports')
    .select('id,title,latitude,longitude,status,created_at,duplicate_of,is_potential_duplicate')
